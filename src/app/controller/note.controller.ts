...
noteRoutes.post('/create-advanced-note',async(req:Request,res:Response)=>{
    //approach1
...
noteRoutes.patch('/:noteId',async(req:Request,res:Response)=>{
    ...
    const note = await appNoteSchema.findByIdAndUpdate(noteId,updatedBody,{new:true})
       res.status(201).json({
        success:true,message:"Advanced note updated successfully.",note
    })
})
